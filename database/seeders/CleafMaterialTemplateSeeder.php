...
/**
 * Cleaf (cleaf.egger.page) — database/data/cleaf_catalog.json.
 * Manufacturer slug: cleaf. Decors apply to both laminate and MDF; sheet size from catalog in cm.
 */
class CleafMaterialTemplateSeeder extends Seeder
{
    public function run(): void
    {
        $path = database_path('data/cleaf_catalog.json');
        if (! is_readable($path)) {
            return;
        }
        ...
        if (! is_array($rows)) {
            throw new \RuntimeException('cleaf_catalog.json must be a JSON array.');
        }
        ...
        foreach ($rows as $index => $row) {
            ...
            $id = Uuid::uuid5(
                Uuid::NAMESPACE_URL,
                'mebel:material-template:cleaf:'.mb_strtoupper($externalCode).':'.$slug.':'.hash('sha256', $fingerprint.':'.$index)
            )->toString();
