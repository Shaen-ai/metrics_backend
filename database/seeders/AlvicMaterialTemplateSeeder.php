...
/**
 * Alvic (alvic.com) — database/data/alvic_catalog.json.
 * Manufacturer slug: alvic. Each decor applies to both laminate (HPL/melamine) and MDF sheet stock.
 * Sheet size from catalog (e.g. 2750 × 1240 mm) is stored as cm on the template.
 */
class AlvicMaterialTemplateSeeder extends Seeder
{
    public function run(): void
    {
        $path = database_path('data/alvic_catalog.json');
        if (! is_readable($path)) {
            return;
        }
        ...
        if (! is_array($rows)) {
            throw new \RuntimeException('alvic_catalog.json must be a JSON array.');
        }
        ...
        foreach ($rows as $index => $row) {
            ...
            $id = Uuid::uuid5(
                Uuid::NAMESPACE_URL,
                'mebel:material-template:alvic:'.mb_strtoupper($externalCode).':'.$slug.':'.hash('sha256', $fingerprint.':'.$index)
            )->toString();
    ...
    /**
     * Parse "2750mm * 1240mm" into sheet width/height in cm (first × second).
     *
